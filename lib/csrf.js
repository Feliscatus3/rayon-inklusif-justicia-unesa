 ...
 *
 * Prefers SESSION_SECRET from the environment. If it is missing or too short,
 * falls back to a per-process random secret so the app NEVER crashes on login.
 * (A per-process secret is still secure for CSRF double-submit protection within
 * a session; it is only rotated across cold starts, which is acceptable.)
 */
function getSecret() {
...
/**
 * Get the plain CSRF token for the current request cookie, WITHOUT the
 * signature (i.e. the value the client must echo in the X-CSRF-Token header).
 *
 * If the request already carries a valid csrf_token cookie, its plain token
 * is returned. Otherwise a fresh token is issued on the response.
 *
 * This is safe: the signature is never exposed; the caller validates the
 * header against the full cookie via validateCsrf() on state changes.
 *
 * @param {import('http').IncomingMessage} req
 * @param {import('http').ServerResponse} res
 * @returns {string|null} plain token, or null if parsing failed
 */
function getCsrfTokenForRequest(req, res) {
  const cookies = parseCookies(req.headers.cookie || '');
  const cookieValue = cookies[COOKIE_NAME];
  const parsed = parseCsrfCookie(cookieValue);
  if (parsed && isValidToken(parsed.token, parsed.signature)) {
    return parsed.token;
  }
  if (typeof res === 'object' && res && typeof res.setHeader === 'function') {
