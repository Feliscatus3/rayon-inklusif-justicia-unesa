 ...
 *
 * RBAC:
 *   GET    /api/announcements          — All authenticated users (view)
 *   GET    /api/announcements/:id      — All authenticated users (view)
 *   POST   /api/announcements          — super_admin, admin, ketua_rayon, sekretaris (create)
 *   PUT    /api/announcements/:id      — super_admin, admin, ketua_rayon, sekretaris (edit)
 *   PATCH  /api/announcements/:id/pin  — super_admin, admin (toggle pin)
 *   DELETE /api/announcements/:id      — super_admin, admin only
 *
 * Schema (migration-v5.sql): announcements(id, title, content, category,
 *   is_pinned, pdf_url, pdf_name, created_by, created_at, updated_at)
 */
...
async function getAnnouncements(req, res) {
  ...
  if (pinned === 'true' || pinned === '1') {
    ...
    sql += ' AND a.is_pinned = false';
  }

  if (search) {
    sql += ` AND (a.title ILIKE $${i++} OR a.content ILIKE $${i++})`;
    const term = `%${search}%`;
...
async function createAnnouncement(req, res, user, ip, ua) {
  ...
  if (body === null) {
    return res.status(400).json({ error: 'Invalid JSON body. Untuk upload PDF gunakan form-data.' });
  }
  ...
  await logAudit({
    userId: user.id, action: 'ANNOUNCEMENT_CREATED', ip, userAgent: ua,
...
async function updateAnnouncement(req, res, user, ip, ua) {
  ...
  const isPinned = body.is_pinned !== undefined
    ? (body.is_pinned === true || body.is_pinned === 'true' || body.is_pinned === '1')
  ...
  const result = await query(
    `UPDATE announcements
     SET title = $1, content = $2, category = $3, is_pinned = $4, updated_at = NOW()
     WHERE id = $5
     RETURNING *`,
  ...
  await logAudit({
    userId: user.id, action: 'ANNOUNCEMENT_UPDATED', ip, userAgent: ua,
    details: JSON.stringify({ id: annId, title: result.rows[0].title })
  });
...
async function togglePin(req, res, user, ip, ua) {
  ...
  await logAudit({
    userId: user.id, action: newPin ? 'ANNOUNCEMENT_PINNED' : 'ANNOUNCEMENT_UNPINNED', ip, userAgent: ua,
    details: JSON.stringify({ id: annId })
  });
...
async function deleteAnnouncement(req, res, user, ip, ua) {
  ...
  await logAudit({
    userId: user.id, action: 'ANNOUNCEMENT_DELETED', ip, userAgent: ua,
    details: JSON.stringify({ id: annId, title: existing.rows[0].title })
  });
